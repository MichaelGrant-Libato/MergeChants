...
const HS_JUNIOR = "Junior High School Department";
const HS_SENIOR = "(SHS) Senior High School Department";

// College-only courses (remove HS entries)
const COLLEGE_COURSES = COURSES.filter(
  (c) => c !== HS_JUNIOR && c !== HS_SENIOR
);
...
export default function Registration() {
  ...
  const isSeniorHS = ["Grade 11", "Grade 12"].includes(formData.yearLevel);
  const isCollege =
    formData.yearLevel &&
    !isJuniorHS &&
    !isSeniorHS &&
    YEAR_LEVELS.includes(formData.yearLevel);
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      // special handling when year level changes
      if (name === "yearLevel") {
        if (["Grade 7", "Grade 8", "Grade 9", "Grade 10"].includes(value)) {
          updated.course = HS_JUNIOR;
        } else if (["Grade 11", "Grade 12"].includes(value)) {
          updated.course = HS_SENIOR;
        } else {
          // college years: reset course so user must pick
          updated.course = "";
        }
      }
  ...
  const validate = () => {
    ...
    if (!studentNumber.trim()) {
      ...
      newErrors.studentNumber =
        "Invalid format. Use XX-XXXX-XXX (e.g., 20-0000-001).";
    }

    if (!outlookEmail.trim()) {
      newErrors.outlookEmail = "Outlook email is required.";
    } else if (!/^[a-zA-Z0-9._%+-]+@cit\.edu$/.test(outlookEmail.trim())) {
      newErrors.outlookEmail = "Use your official @cit.edu email only.";
    }
    ...
    if (
      middleInitial.trim() &&
      !/^[A-Za-z.\s]{1,3}$/.test(middleInitial.trim())
    ) {
    ...
    if (password.length < 8) {
      newErrors.password = "Min 8 chars (upper, lower, number).";
  ...
  // "Sign in here" click -> show loading, go to login
        ...
        <div className="registration-overlay">
          <div className="registration-card">
            ...
            <form className="registration-form" onSubmit={handleSubmit}>
              ...
              {/* Outlook Email */}
              <div className="form-group">
                <label htmlFor="outlookEmail">CIT Outlook Email</label>
                <input
                  type="email"
                  id="outlookEmail"
                  name="outlookEmail"
                  ...
                  onChange={handleChange}
                  placeholder="e.g., your.name@cit.edu"
                  className={errors.outlookEmail ? "input-error" : ""}
                />
                {errors.outlookEmail && (
                  <p className="error">{errors.outlookEmail}</p>
                )}
              </div>
              ...
              <div className="name-row">
                ...
                <div className="form-group half-width">
                  ...
                  {errors.middleInitial && (
                    <p className="error">{errors.middleInitial}</p>
                  )}
                </div>
              </div>
              ...
              <div className="name-row">
                ...
                <div className="form-group half-width">
                  ...
                  {errors.yearLevel && (
                    <p className="error">{errors.yearLevel}</p>
                  )}
                </div>
              </div>

              {/* Course/Department: hidden until a year level is selected */}
              {formData.yearLevel && (
                <div className="form-group">
                  ...
                  {isJuniorHS && (
                    <input
                      ...
                      id="course"
                      name="course"
                      value={formData.course || HS_JUNIOR}
                      readOnly
                  ...
                  {isSeniorHS && (
                    <input
                      ...
                      id="course"
                      name="course"
                      value={formData.course || HS_SENIOR}
                      readOnly
                      className="readonly-field"
                    />
                  )}
